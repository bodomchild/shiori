...
const cityCharacters = ['MZA', '大阪', '京都', '金沢', '河口湖', '箱根', '東京'];
...
export default function TripPage() {
  ...
  const currentLabel = current.relation === 'today'
    ...
    : current.relation === 'before'
      ? 'Ir al primer día'
      : current.relation === 'upcoming'
        ? 'Ir al próximo día'
        : 'Volver al último día';
  return <div className="page-shell">
    <section className="trip-hero">
      <div className="hero-copy">
        <p className="eyebrow"><span className="red-dot" /> SEPTIEMBRE — NOVIEMBRE 2026</p>
        ...
        <div className="hero-meta"><span>13 sep — 4 nov</span><span>{totalDays} días</span><span>{itinerary.cities.length} ciudades</span></div>
    ...
    <section className="pb-8" aria-labelledby="cities-title">
      <div className="section-heading"><div><p className="eyebrow">DE MENDOZA A TOKIO</p><h2 id="cities-title">Nuestro recorrido</h2></div><span className="small-muted">{String(itinerary.cities.length).padStart(2, '0')} destinos</span></div>
    ...
    <aside className="sample-note"><span className="note-symbol" aria-hidden="true">栞</span><div><strong>El itinerario ya está con nosotros</strong><p>Los {totalDays} días están cargados localmente. Mendoza es un bloque temporal de prueba; los días de Japón mantienen el contenido extraído de la fuente.</p></div><Link to={`/osaka/${daySlug('2026-10-13')}`} className="text-link">Ir a Japón <span aria-hidden="true">→</span></Link></aside>
  </div>;
}
